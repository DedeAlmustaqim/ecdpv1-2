<?php
class M_ijinsita extends CI_Model
{

    public $table = 'tbl_ijinsita';
    public $id = 'id';
    public $order = 'asc';
    ...
    public function json_ijinsita()
    {
        ...
        ');
        $this->datatables->from('tbl_ijinsita');
        if ($this->session->userdata('akses') == 4) {
            $this->datatables->where('tbl_ijinsita.id_unit', $where);
        }
        $this->datatables->join('tbl_unit', 'tbl_ijinsita.id_unit = tbl_unit.id_unit', 'left');
    ...
    public function json_brg_sita($id)
    {
        ...
        $this->datatables->select('*');
        $this->datatables->from('tbl_brg_ijin_sita');
    ...
    {
        return $this->db->insert('tbl_ijinsita', $data);
    }
    ...
    function update($data)
    {
        ...
        $result = $this->db->update('tbl_ijinsita', $data);
        return $result;
    }
    public function insert_brg($data)
    {
        return $this->db->insert('tbl_brg_ijin_sita', $data);
    }
