...
class Ijinsita extends CI_Controller
{
	...
	{
		$this->load->library('template');
		$data['judul'] = "Penyitaan - Penyidikan - Ijin Sita";
		$data['unit'] = $this->m_master->tampil_unit();
		$this->template->konten('penyitaan/ijin_sita', $data);
		//var_dump($data);
	}



	public function json_sita()
	{
		$data = $this->m_ijinsita->json_ijinsita();
	...
	{
		$row = $this->m_ijinsita->get_id_ijin_sita($id);
		if ($row) {
			$data = array(
	...
	public function tambah_st()
	{
		...
		$edoc1 = $_FILES['edoc_st1']['name'];

		$config1['upload_path']          = 'file/st/ijinsita/';
		...
		if (!$this->upload->do_upload('edoc_st1')) {
			$array = array(
				'error'   => true,
			);
			echo json_encode($array);
		...
		// image was uploaded properly, continue
			...
			$edoc2 = $_FILES['edoc_st2']['name'];

			$config2['upload_path']          = 'file/st/ijinsita/';
			...
			if (!$this->upload->do_upload('edoc_st2')) {
				$array = array(
					'error'   => true,
				);
				echo json_encode($array);
				...
				$edoc3 = $_FILES['edoc_st3']['name'];

				$config3['upload_path']          = 'file/st/ijinsita/';
				...
				if (!$this->upload->do_upload('edoc_st3')) {
					$array = array(
						'error'   => true,
					);
					echo json_encode($array);
					...
					$edoc4 = $_FILES['edoc_st4']['name'];

					$config4['upload_path']          = 'file/st/ijinsita/';
					$config4['allowed_types']        = 'pdf';
					$x4 = explode(".", $edoc4);
					$ext4 = strtolower(end($x4));
					$config4['file_name'] = $id . "_perintah." . $ext4;
					$config4['overwrite']            = true;
					$config4['max_size']             = 3072; // 1MB
					$edoc4_name = $config4['file_name'];
					$this->upload->initialize($config4);

					if (!$this->upload->do_upload('edoc_st4')) {
						$array = array(
							'error'   => true,
						);
						echo json_encode($array);
                        ...
                        if (!$this->upload->do_upload('edoc_st5')) {
                            ...
                            $no_smohon_sita  		= $this->input->post('no_smohon_sita', true);
                            $unit_st 			= $this->input->post('unit_st', true);
	...
	public function delete_st()
	{
		if ($this->cek_delete_st()) {
			...
			$this->db->where('id_ijin_sita', $id);
			$this->db->delete('tbl_ijinsita');
			unlink(FCPATH . 'file/st/ijinsita/' . $smohon);
			unlink(FCPATH . 'file/st/ijinsita/' . $lap);
			unlink(FCPATH . 'file/st/ijinsita/' . $penetapan);
			unlink(FCPATH . 'file/st/ijinsita/' . $sp);
			die;
		} else {
			redirect('ijinsita');
		}
	...
	public function delete_brg()
	{
		if ($this->cek_delete_brg()) {
			...
			$this->db->delete('tbl_brg_ijin_sita');
		} else {
			redirect('ijinsita');
		}
	}
	...
	public function tambah_brg()
	{
		...
		if ($this->cek_tambah_brg()) {
			...
			$this->m_ijinsita->insert_brg($data);
			$array = array(
				'success' => 'sukses'
			);
