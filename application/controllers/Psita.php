...
class Psita extends CI_Controller
{
	...
	{
		$this->load->library('template');
		$data['judul'] = "Penyitaan - Penyidikan - Persetujuan Sita";
		$data['unit'] = $this->m_master->tampil_unit();
		$this->template->konten('penyitaan/persetujuan_sita', $data);
		//var_dump($data);
	}

	public function json_psita()
	{
		$data = $this->m_psita->json_psita();
	...
	public function valid()
	{
		...
		$id      	= $this->input->post('id_verif_pst', true);
		$verif = $_FILES['verif_pst']['name'];

		$config['upload_path']          = 'file/st/psita';
	...
	public function buka_valid()
	{
		...
		if ($this->cek_buka_valid()) {
			...
			$this->m_psita->valid($data);
			unlink(FCPATH . 'file/st/psita' . $edoc);
			echo json_encode($data);
	...
	{
		$row = $this->m_psita->get_id_psita($id);
		if ($row) {
			$data = array(
	...
	public function tambah_pst()
	{
		...
		$config1['upload_path']          = 'file/st/psita/';
		...
		// image was uploaded properly, continue
			...
			$config2['upload_path']          = 'file/st/psita/';
			...
			if (!$this->upload->do_upload('edoc_pst2')) {
				...
				$config3['upload_path']          = 'file/st/psita/';
				...
				if (!$this->upload->do_upload('edoc_pst3')) {
					...
					$edoc4 = $_FILES['edoc_pst4']['name'];

					$config4['upload_path']          = 'file/st/psita/';
					...
					if (!$this->upload->do_upload('edoc_pst4')) {
						$array = array(
							'error'   => true,
						);
						echo json_encode($array);
						...
						$config5['upload_path']          = 'file/st/psita/';
						...
						if (!$this->upload->do_upload('edoc_pst5')) {
							...
							$config6['upload_path']          = 'file/st/psita/';
							...
							if (!$this->upload->do_upload('edoc_pst6')) {
								...
								$config7['upload_path']          = 'file/st/psita/';
								...
								if (!$this->upload->do_upload('edoc_pst7')) {
									...
									$config8['upload_path']          = 'file/st/psita/';
	...
	public function delete_pst()
	{
		if ($this->cek_delete_pst()) {
			...
			$this->db->where('id_psita', $id);
			$this->db->delete('tbl_psita');
			unlink(FCPATH . 'file/st/psita/' . $smohon);
			unlink(FCPATH . 'file/st/psita/' . $lap);
			unlink(FCPATH . 'file/st/psita/' . $penetapan);
			unlink(FCPATH . 'file/st/psita/' . $spdp);
			unlink(FCPATH . 'file/st/psita/' . $sp);
			unlink(FCPATH . 'file/st/psita/' . $ba);
			unlink(FCPATH . 'file/st/psita/' . $ttba);
			unlink(FCPATH . 'file/st/psita/' . $sprindik);
