...
class Penahanan_p extends CI_Controller
{
	...
	{
		$this->load->library('template');
		$data['judul'] = "Penahanan - Penuntutan";
		$data['unit'] = $this->m_master->tampil_unit();
		$this->template->konten('penahanan/penahanan_p', $data);
		//var_dump($data);
	}

	public function json_penahanan()
	{
		$data = $this->m_penahanan_p->json_penahanan();
	...
	public function valid()
	{
		...
		$id      	= $this->input->post('id_verif_php', true);
		$verif = $_FILES['verif_php']['name'];

		$config['upload_path']          = 'file/ph-p';
		...
		if (!$this->upload->do_upload('verif_php')) {
			$array = array(
				'error'   => true,
			);
			echo json_encode($array);
		} else {
			$data = array('verif_php' => $this->upload->data());
			$data = array(
			...
			$this->m_penahanan_p->verif($data, $id);
			$array = array(
				'success' => 'sukses'
			);
			echo json_encode($array);
	...
	public function buka_valid()
	{
		...
		if ($this->cek_buka_valid()) {
			...
			$this->m_penahanan_p->valid($data);
			echo json_encode($data);
			unlink(FCPATH . 'file/ph-p/' . $this->input->post('edoc'));
	...
	{
		$row = $this->m_penahanan_p->get_ph($id);
		if ($row) {
			$data = array(
	...
	public function tambah_penahanan()
	{
		...
		// image was uploaded properly, continue
						...
						$kebangsaan_ph  	= $this->input->post('kebangsaan_ph', true);
							...
							$this->m_penahanan_p->insert($data);
	...
	public function edit_ph()
	{
		...
		$this->m_penahanan_p->update($data, $id_ph);
		$array = array(
			'success' => 'sukses'
		);
	...
	public function reupload_resume()
	{

		$this->load->helper(['form', 'string']);
		$id_edoc5      	= $this->input->post('id_edoc5', true);
		$edoc5 = $_FILES['edoc5_ph_edit']['name'];

		$config['upload_path']          = 'file/ph-p/';
		$config['allowed_types']        = 'pdf';
		$x = explode(".", $edoc5);
		$ext = strtolower(end($x));
		$config['file_name'] = $id_edoc5 . "_resume." . $ext;
		$config['overwrite']            = true;
		$config['max_size']             = 5024; // 1MB
		// $config['max_width']            = 1024;
		// $config['max_height']           = 768;
		...
		if (!$this->upload->do_upload('edoc5_ph_edit')) {
			$array = array(
				'error'   => true,
			);
			echo json_encode($array);
		} else {
			array('edoc5_ph_edit' => $this->upload->data());
	...
	public function delete_ph()
	{
		if ($this->cek_delete_ph()) {
			...
			$this->db->delete('tbl_penahanan_p');
			unlink(FCPATH . 'file/ph-p/' . $smohon);
	...
	public function edoc_r()
	{
		...
		$id      	= $this->input->post('id_r');
			...
			if (!$this->upload->do_upload('edoc_r2')) {
				...
				$this->m_penahanan_p->update_r($data, $id);
				$array = array(
					'success' => 'sukses'
				);
